...
async function hashVisitor(visitorId, env) {
  const input = new TextEncoder().encode(`${env.TRAFFIC_HASH_SALT}|${visitorId}`)
...
function configState(env) {
  const databaseConfigured = Boolean(env.PORTFOLIO_DB)
  const hashSaltConfigured = Boolean(env.TRAFFIC_HASH_SALT)
  let reason = null
  if (!databaseConfigured) reason = "missing_database_binding"
  else if (!hashSaltConfigured) reason = "missing_hash_salt"
  return {
    configured: databaseConfigured && hashSaltConfigured,
    databaseConfigured,
    hashSaltConfigured,
    reason,
  }
...
export async function onRequestPost({ request, env }) {
  ...
  if (!state.configured) {
    return json({
      ...state,
      error: state.reason === "missing_database_binding"
        ? "PORTFOLIO_DB binding belum tersedia."
  ...
  if (event === "pageview") {
    await env.PORTFOLIO_DB.prepare(`
      INSERT INTO traffic_daily (day, visitor_hash, first_seen, last_seen, pageviews)
      VALUES (?, ?, ?, ?, 1)
      ON CONFLICT(day, visitor_hash)
      DO UPDATE SET
        last_seen = excluded.last_seen,
        pageviews = traffic_daily.pageviews + 1
    `).bind(day, visitorHash, now, now).run()
  } else {
    await env.PORTFOLIO_DB.prepare(`
      INSERT INTO traffic_daily (day, visitor_hash, first_seen, last_seen, pageviews)
      VALUES (?, ?, ?, ?, 1)
  ...
  return json({ ...state, ok: true, event, day })
}

export async function onRequestGet({ env }) {
  const state = configState(env)
  if (!env.PORTFOLIO_DB) {
    return json({ ...state, current: 0, today: 0, week: 0, month: 0, year: 0, series: [] })
  }
  ...
  const [current, todayCount, weekCount, monthCount, yearCount, seriesResult] =
    await Promise.all([
      getCount(env.PORTFOLIO_DB,
        "SELECT COUNT(*) AS count FROM traffic_presence WHERE last_seen >= ?",
        now - ACTIVE_WINDOW_SECONDS),
      getCount(env.PORTFOLIO_DB,
        "SELECT COUNT(*) AS count FROM traffic_daily WHERE day = ?", today),
      getCount(env.PORTFOLIO_DB,
      ...
      env.PORTFOLIO_DB.prepare(`
        SELECT day, COUNT(*) AS visitors, SUM(pageviews) AS pageviews
        FROM traffic_daily
  ...
  const rows = new Map((seriesResult.results || []).map((row) => [
    row.day,
    { visitors: Number(row.visitors || 0), pageviews: Number(row.pageviews || 0) },
  ]))
  ...
  for (let index = 0; index < 30; index += 1) {
    ...
    series.push({ day, visitors: value.visitors, pageviews: value.pageviews })
  }
